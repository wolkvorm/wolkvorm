...
func refreshOutputsHandler(w http.ResponseWriter, r *http.Request) {
	...
	if getExecutionMode() == "kubernetes" {
		...
		logs, _ := RunKubeJob(KubeJobOpts{
			...
			Args:       []string{"-c", "cd /workspace && " + iacBinary() + " init -no-color -input=false && " + iacBinary() + " output -json"},
			EnvVars:    envVars,
			HCLContent: hcl,
		})
		...
		args = append(args,
			...
			"-v", runDir+":/workspace",
			"wolkvorm-runner",
			"-c", "cd /workspace && " + iacBinary() + " init -no-color -input=false && " + iacBinary() + " output -json",
		)
